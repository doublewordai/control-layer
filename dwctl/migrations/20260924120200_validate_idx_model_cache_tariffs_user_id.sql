...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE i.indexrelid = to_regclass(format('%I.idx_model_cache_tariffs_user_id', current_schema()))
        ...
        RAISE EXCEPTION 'idx_model_cache_tariffs_user_id is missing, invalid, not ready, or has the wrong definition; repair the concurrent build before upgrading';
    END IF;
END
$$;

COMMENT ON INDEX idx_model_cache_tariffs_user_id IS 'Supports organisation-scoped tariff lookup and account foreign-key checks.';
