...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE i.indexrelid = to_regclass(format('%I.idx_model_cache_tariffs_version_org', current_schema()))
          ...
          AND pg_get_indexdef(i.indexrelid, 3, true) = 'valid_from'
        ...
        RAISE EXCEPTION 'idx_model_cache_tariffs_version_org is missing, invalid, not ready, or has the wrong definition; repair the concurrent build before upgrading';
    END IF;
END
$$;

COMMENT ON INDEX idx_model_cache_tariffs_version_org IS 'Preserves cache-tariff ledger uniqueness independently for general, organisation and class scopes.';
