...
/// granted. `standard` is the absence of a choice and is never declared.
...
impl Catalog {
    ...
    fn validate(&self) -> Result<()> {
        ...
        for model in &self.models {
            ...
            for class in &model.clay.serving_classes {
                ensure!(
                    classes.insert(*class),
                    "{}: duplicate serving class {:?}",
                    model.source,
                    class.as_db_str()
                );
            }
...
"#
    ...
    async fn apply_materialises_active_classes_and_overlays(pool: PgPool) {
        ...
        let classes: Vec<String> = sqlx::query_scalar("SELECT serving_classes FROM deployed_models WHERE alias = 'org/model'")
            ...
            .await
            .unwrap();
        assert_eq!(classes, vec!["interactive".to_string()]);
    }
