...
mod tests {
    ...
    mod credit_deduction_tests {
        ...
        async fn test_credit_deduction_successful(pool: sqlx::PgPool) {
            ...
            let transactions = credits.list_user_transactions(user_id, 0, 10, None, None).await.unwrap();
        ...
        async fn test_concurrent_requests_with_consistent_pricing(pool: sqlx::PgPool) {
            ...
            let mut conn = pool.acquire().await.unwrap();
            let mut credits = Credits::new(&mut conn);
            let transactions = credits.list_user_transactions(user_id, 0, 20, None, None).await.unwrap();
