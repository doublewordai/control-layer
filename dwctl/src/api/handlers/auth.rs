...
pub struct CliCallbackQuery {
    /// Localhost port where the CLI is listening.
    pub port: u16,
    /// CSRF state token generated by the CLI.
    pub state: String,
    /// Optional org slug to create org-scoped keys.
    pub org: Option<String>,
}

/// CLI login callback handler.
///
/// Called after the user authenticates via SSO. Creates a pair of API keys
/// (inference + platform) and redirects to the CLI's localhost server with
/// the keys and user info as query parameters.
///
/// Flow:
///   1. CLI opens browser → SSO → oauth2-proxy sets cookie → redirects here
///   2. This handler authenticates the user from the SSO cookie
///   3. Creates two external API keys directly via the DB repository
...
pub async fn cli_callback<P: PoolProvider>(
    ...
    let (target_user_id, account_name, org_id_param) = if let Some(ref org_slug) = query.org {
        ...
        // Get org details to match by slug (username)
        ...
        let matched_org = memberships.iter().find_map(|m| {
            org_map.get(&m.organization_id).and_then(|org| {
                // Match by username (slug) or display_name, case-insensitive
    ...
    // Create two external API keys via the DB repository
    let mut pool_conn = state.db.write().acquire().await.map_err(|e| Error::Database(e.into()))?;
    let mut repo = ApiKeys::new(&mut pool_conn);

    let inference_key_req = crate::db::models::api_keys::ApiKeyCreateDBRequest {
    ...
    let inference_key = repo.create(&inference_key_req).await.map_err(Error::Database)?;

    let platform_key_req = crate::db::models::api_keys::ApiKeyCreateDBRequest {
        user_id: target_user_id,
    ...
    let platform_key = repo.create(&platform_key_req).await.map_err(Error::Database)?;

    // Build redirect URL to CLI's localhost callback
    let mut redirect_url = format!(
        "http://127.0.0.1:{}/callback?state={}&inference_key={}&inference_key_id={}&platform_key={}&platform_key_id={}&user_id={}&email={}&display_name={}&account_name={}",
        query.port,
        urlencoding::encode(&query.state),
        urlencoding::encode(&inference_key.secret),
        inference_key.id,
        urlencoding::encode(&platform_key.secret),
        platform_key.id,
        target_user_id,
        urlencoding::encode(&user_email),
        urlencoding::encode(&user_display_name),
        urlencoding::encode(&account_name),
    );

    if let Some(ref org_id) = org_id_param {
        redirect_url.push_str(&format!("&org_id={}", org_id));
    }

    // Return an HTML page that redirects to localhost and shows a "close this tab" message
    let html = format!(
        r#"<!DOCTYPE html>
<html>
<head>
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; display: flex; justify-content: center; align-items: center; height: 100vh; margin: 0; background: #fafafa;">
    <div style="text-align: center; max-width: 400px;">
        ...
        <p style="color: #999; font-size: 0.9em;">If nothing happens, <a href="{redirect_url}">click here</a>.</p>
    </div>
