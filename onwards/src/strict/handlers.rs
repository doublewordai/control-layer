...
struct ForwardResult {
    response: Response,
    trusted: bool,
    authenticated_api_key_id: Option<AuthenticatedApiKeyId>,
...
fn with_authenticated_api_key_id(
    mut response: Response,
    authenticated_api_key_id: Option<AuthenticatedApiKeyId>,
) -> Response {
    if let Some(api_key_id) = authenticated_api_key_id {
        response.extensions_mut().insert(api_key_id);
    }
    response
}
...
pub async fn chat_completions_handler<T: HttpClient + Clone + Send + Sync + 'static>(
    ...
    with_authenticated_api_key_id(response, authenticated_api_key_id)
}

/// Handler for POST /v1/responses
///
/// Validates the request against the Open Responses schema and forwards it to
/// the upstream provider as-is. The upstream is expected to speak the Responses
/// API natively; translating Responses onto a Chat Completions backend is the
/// edge's job, not onwards' (see the alias branch below).
pub async fn responses_handler<T: HttpClient + Clone + Send + Sync + 'static>(
    ...
    // Reject malformed canonical reasoning controls here, with a Responses-shaped
    // error, rather than letting the upstream produce an arbitrary one.
...
async fn forward_request<T: HttpClient + Clone + Send + Sync + 'static>(
    ...
    let trusted = response
        ...
        .map(|t| t.0)
        .unwrap_or(false);
    let authenticated_api_key_id = response
        .extensions()
        .get::<AuthenticatedApiKeyId>()
        .copied();
