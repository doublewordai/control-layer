...
use super::schemas::responses::{
    ResponsesRequest, ResponsesResponse, ResponsesStreamingEvent, generated_response_id,
    normalize_responses_response_value, normalize_responses_streaming_event_value,
};
...
pub async fn chat_completions_handler<T: HttpClient + Clone + Send + Sync + 'static>(
    ...
    chat_completions_handler_inner(state, headers, request, None).await
}

async fn chat_completions_handler_inner<T: HttpClient + Clone + Send + Sync + 'static>(
    state: AppState<T>,
    headers: HeaderMap,
    mut request: ChatCompletionRequest,
    continuation_eligibility_path: Option<&'static str>,
) -> Response {
    ...
    let ForwardResult {
        ...
        trusted,
        internal_error,
    } = forward_request_with_eligibility(
        state,
        headers,
        "/chat/completions",
        body_bytes,
    ...
    if response.status().is_success() {
        ...
        let preserve_encoded_stream = response
            .extensions()
            .get::<crate::handlers::PreserveEncodedStream>()
            .is_some();
        ...
        if (is_streaming || response_is_sse) && !preserve_encoded_stream {
...
pub async fn responses_handler<T: HttpClient + Clone + Send + Sync + 'static>(
    ...
    if req.uri().path().ends_with("/chat/completions") {
        return match Json::<ChatCompletionRequest>::from_request(req, &state).await {
            Ok(Json(chat_request)) => {
                chat_completions_handler_inner(state, headers, chat_request, Some("/v1/responses"))
                    .await
            }
    ...
    let canonical_reasoning =
        match crate::reasoning::validate_canonical_reasoning("/responses", &request_value) {
            Ok(reasoning) => reasoning,
            Err(error) => return OnwardsErrorResponse::reasoning(&error).into_response(),
        };
    ...
    // Error responses are only sanitized for untrusted providers
    // Internal errors (generated by onwards itself) are never sanitized
    let final_response = if response.status().is_success() {
        let response_is_sse = response_is_sse(&response);
        let preserve_encoded_stream = response
            .extensions()
            .get::<crate::handlers::PreserveEncodedStream>()
        ...
        .body(Body::from(body_bytes))
    {
        Ok(mut req) => {
            *req.headers_mut() = headers;
            req.extensions_mut()
                .insert(crate::handlers::ContinuationEligibilityPath(
                    "/v1/responses",
                ));
            if let Some(reasoning) = canonical_reasoning {
                req.extensions_mut().insert(reasoning.clone());
            }
            req
        }
        Err(e) => {
...
async fn forward_request<T: HttpClient + Clone + Send + Sync + 'static>(
    ...
    forward_request_with_eligibility(state, headers, path, body_bytes, None).await
}

async fn forward_request_with_eligibility<T: HttpClient + Clone + Send + Sync + 'static>(
    state: AppState<T>,
    mut headers: HeaderMap,
    path: &str,
    body_bytes: Vec<u8>,
    ...
    let request = match request_builder.body(Body::from(body_bytes)) {
        Ok(mut req) => {
            ...
            if let Some(canonical_path) = canonical_path {
                req.extensions_mut()
                    .insert(crate::handlers::CanonicalRequestPath(canonical_path));
            }
            if let Some(eligibility_path) = continuation_eligibility_path {
                req.extensions_mut()
                    .insert(crate::handlers::ContinuationEligibilityPath(
                        eligibility_path,
                    ));
...
async fn sanitize_streaming_chat_response(
    ...
    let buffer_limit = response
        .extensions()
        .get::<crate::handlers::SseBufferLimit>()
        .map(|limit| limit.0);
    let body_stream =
        http_body_util::BodyExt::into_data_stream(std::mem::take(response.body_mut()));
    let buffered_stream = match buffer_limit {
        Some(limit) => crate::sse::CheckedSseStream::with_max_buffer_size(body_stream, limit),
        None => crate::sse::CheckedSseStream::new(body_stream),
    };
...
"#
    ...
    #[tokio::test]
    async fn test_strict_responses_budget_error_preserves_responses_parameter_names() {
        ...
        let request = Request::builder()
            ...
            .header("content-type", "application/json")
            .body(Body::from(
                r#"{"model":"reasoning-model","input":"Hello","reasoning":{"effort":"high"},"max_output_tokens":4096}"#,
            ))
            .unwrap();
    ...
    #[tokio::test]
    async fn test_strict_responses_adapter_applies_reasoning_effort_and_budget() {
        let reasoning_translation = serde_json::from_value(json!({
            "chat_completions": {
                "unsupported_efforts": ["none", "minimal", "low", "medium", "xhigh", "max"],
                "writes": [
        ...
        let mock_client = MockHttpClient::new(StatusCode::OK, &mock_response.to_string());
        let state = AppState::with_client(targets, mock_client.clone());
        let router = crate::strict::build_strict_router(state);
        let request = Request::builder()
            .method("POST")
            ...
            .header("content-type", "application/json")
            .body(Body::from(
                r#"{"model":"reasoning-model","input":"Hello","reasoning":{"effort":"high"},"max_output_tokens":9000}"#,
            ))
            .unwrap();
        ...
        assert_eq!(upstream_body["thinking_token_budget"], 8192);
        assert_eq!(upstream_body["max_tokens"], 9000);
    }
