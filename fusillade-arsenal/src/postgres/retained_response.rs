...
/// Timeout for the `EXPLAIN` fallback estimate. Unlike the `COUNT(*)` scans it
/// replaces, `EXPLAIN (FORMAT JSON)` is planning-only — it never walks the
    ...
    for arm in [LIVE_REQUEST_COUNT_SQL, RETAINED_REQUEST_COUNT_SQL] {
        ...
        total += match exact {
            ...
            Err(sqlx::Error::Database(e)) if e.code().as_deref() == Some("57014") => {
                ...
                // fresh one (EXPLAIN is planning only). The plan itself needs
                // more room than the COUNT scans (see EXPLAIN_TIMEOUT), so set
                // that here rather than COUNT_BUDGET.
                ...
                match plan {
                    Ok(plan) => plan
                        ...
                        .unwrap_or(0),
                    // An estimate is not worth failing the listing over: on the
                    // rare path where even planning exceeds the generous
