...
/// A `PoolProvider` that routes both reads and writes to one pool, so a
/// table lock taken on any connection is visible to every query issued
/// through the manager (no replica to obscure it).
#[derive(Clone)]
