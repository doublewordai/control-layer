...
import { validateBatchFile, FILE_SIZE_LIMITS } from "./files";
...
describe("files", () => {
  describe("validateBatchFile", () => {
    ...
    it("should reject files without extension", () => {
      const noExtFile = new File(["content"], "test", {
    ...
    it("should reject files exceeding max size", () => {
      const largeContent = new ArrayBuffer(
        FILE_SIZE_LIMITS.MAX_FILE_SIZE_BYTES + 1,
      );
      const largeFile = new File([largeContent], "large.jsonl", {
        type: "application/jsonl",
      });
    ...
    it("should accept files at exactly max size", () => {
      ...
      const maxFile = new File([maxContent], "max.jsonl", {
        type: "application/jsonl",
      });
    ...
    it("should accept files just under max size", () => {
      const justUnderContent = new ArrayBuffer(
        FILE_SIZE_LIMITS.MAX_FILE_SIZE_BYTES - 1,
      );
      const justUnderFile = new File([justUnderContent], "just-under.jsonl", {
        type: "application/jsonl",
      });
    ...
    it("should accept large warning threshold files", () => {
      const largeWarningContent = new ArrayBuffer(
        FILE_SIZE_LIMITS.LARGE_FILE_WARNING_BYTES,
      );
      const largeWarningFile = new File(
        [largeWarningContent],
        "large-warning.jsonl",
        { type: "application/jsonl" },
      );
    ...
    it("should accept files with uppercase extension (case-insensitive)", () => {
      const upperFile = new File(["content"], "test.JSONL", {
        type: "application/jsonl",
      });
    ...
    it("should accept files with mixed case extension", () => {
      const mixedFile = new File(["content"], "test.JsonL", {
        type: "application/jsonl",
      });
    ...
    it("should handle empty files", () => {
      const emptyFile = new File([], "empty.jsonl", {
        type: "application/jsonl",
      });
    ...
    it("should handle files with multiple dots in filename", () => {
      const multiDotFile = new File(["content"], "test.file.name.jsonl", {
        type: "application/jsonl",
      });
  ...
  describe("FILE_SIZE_LIMITS", () => {
    ...
    it("should have warning threshold less than max size", () => {
      expect(FILE_SIZE_LIMITS.LARGE_FILE_WARNING_BYTES).toBeLessThan(
        FILE_SIZE_LIMITS.MAX_FILE_SIZE_BYTES,
      );
      expect(FILE_SIZE_LIMITS.LARGE_FILE_WARNING_MB).toBeLessThan(
        FILE_SIZE_LIMITS.MAX_FILE_SIZE_MB,
      );
    });
