(async function () {
  !(function (t, e) {
    ...
    e.__SV ||
      ...
      (e.init = function (i, s, a) {
        ...
        o =
          "init capture identify alias people.set people.set_once people.increment people.append people.remove people.track_charge people.clear_charges people.delete_user".split(
            " ",
          );
  ...
  posthog.init("phc_Gsc7iKs7KCkxySkNRu8pmJb3dYr4RcrOgYPvnBmvviH", {
  ...
  try {
    const userReq = await fetch("/admin/api/v1/users/current");
    const userJson = await userReq.json();
    if (!userJson.id || !userJson.username) return;
    const dbId = userJson.id;
    const uname = userJson.username;

    posthog.alias(uname, dbId);
    posthog.identify(uname, {
      db_id: dbId,
      last_login: new Date().toISOString(),
    });
  } catch (error) {
    console.warn("Failed to fetch user for PostHog:", error);
  }
})();
