// --- Bootstrap Banner Content ---
const colors = {
  primary: "#2563eb",
  primaryLight: "rgba(37, 99, 235, 0.1)",
  primaryLighter: "rgba(37, 99, 235, 0.05)",
  primaryBorder: "rgba(37, 99, 235, 0.3)",
  primaryBorderLight: "rgba(37, 99, 235, 0.2)",
  background: "#ffffff",
  backgroundSecondary: "#fafaf9",
  foreground: "#2e2c26",
  muted: "#938f78",
  border: "#e2e0d3",
};

var bootstrapContent = `
  <style>
    .dw-bb {
      position: relative;
      border-radius: 0.75rem;
      border: 1px solid ${colors.primaryBorder};
      background: ${colors.background};
      padding: 1.5rem;
      overflow: hidden;
      font-family: 'Space Grotesk', ui-sans-serif, system-ui, sans-serif;
      font-size: 0.875rem;
      color: ${colors.foreground};
    }
    .dw-bb *, .dw-bb *::before { box-sizing: border-box; }
    .dw-bb p, .dw-bb h2, .dw-bb h3 { margin: 0; }
    .dw-glow {
      display: none;
    }
    .dw-content {
      position: relative;
      display: flex;
      flex-direction: column;
      gap: 1rem;
    }
    .dw-heading {
      font-size: 1.25rem;
      font-weight: 600;
      margin-bottom: 0.5rem;
      display: flex;
      align-items: center;
      gap: 0.5rem;
    }
    .dw-dot {
      display: none;
    }
    .dw-muted { color: ${colors.muted}; }
    .dw-hl { color: ${colors.primary}; font-weight: 500; }
    .dw-badges { display: flex; flex-wrap: wrap; gap: 0.75rem; }
    .dw-badge {
      display: flex;
      align-items: center;
      gap: 0.5rem;
      border-radius: 9999px;
      background: rgba(255,255,255,0.8);
      backdrop-filter: blur(4px);
      border: 1px solid ${colors.primaryBorderLight};
      padding: 0.375rem 0.75rem;
      box-shadow: 0 1px 2px rgba(0,0,0,0.05);
    }
    .dw-badge svg { width: 1rem; height: 1rem; color: ${colors.primary}; }
    .dw-cards {
      display: grid;
      grid-template-columns: 1fr;
      gap: 0.75rem;
      margin-top: 0.5rem;
    }
    @media (min-width: 768px) {
      .dw-cards { grid-template-columns: repeat(4, 1fr); }
    }
    .dw-card {
      display: flex;
      flex-direction: row;
      align-items: center;
      gap: 1rem;
      border-radius: 0.5rem;
      border: 1px solid ${colors.border};
      background: rgba(255,255,255,0.7);
      backdrop-filter: blur(4px);
      padding: 1rem;
      text-decoration: none;
      color: inherit;
      transition: border-color 0.2s;
    }
    @media (min-width: 768px) {
      .dw-cards .dw-card {
        flex-direction: column;
        align-items: flex-start;
        gap: 0.5rem;
        position: relative;
      }
      .dw-cards .dw-card-icon {
        width: auto; height: auto;
        background: none;
        border-radius: 0;
      }
      .dw-cards .dw-card-arrow {
        position: absolute;
        top: 1rem; right: 1rem;
        margin-left: 0;
      }
    }
    .dw-card:hover { border-color: rgba(37,99,235,0.5); }
    .dw-card:hover .dw-card-t { color: ${colors.primary}; }
    .dw-card:hover .dw-card-arrow { opacity: 1; }
    .dw-card-icon {
      display: flex;
      align-items: center;
      justify-content: center;
      width: 2.5rem; height: 2.5rem;
      border-radius: 0.5rem;
      background: ${colors.backgroundSecondary};
      flex-shrink: 0;
    }
    .dw-card-icon svg {
      width: 1.25rem; height: 1.25rem;
      color: ${colors.primary};
    }
    .dw-card-arrow {
      font-size: 0.75rem;
      color: ${colors.muted};
      opacity: 0;
      transition: opacity 0.2s;
      margin-left: auto;
      flex-shrink: 0;
    }
    .dw-card-t {
      font-weight: 500;
      color: ${colors.foreground};
      transition: color 0.2s;
    }
    .dw-card-d {
      font-size: 0.75rem;
      color: ${colors.muted};
      margin-top: 0.125rem;
    }

  </style>
  <div class="dw-bb">
    <div class="dw-glow dw-glow--top"></div>
    <div class="dw-glow dw-glow--bottom"></div>
    <div class="dw-content">
      <div>
        <h2 class="dw-heading"><span class="dw-dot"></span>Async inference for workloads at scale</h2>
        <p class="dw-muted">Run batch inference at up to <span class="dw-hl">10x lower cost</span> than real-time APIs. Perfect for data processing, content generation, model evals, and any task that doesn't need an immediate response.</p>
      </div>
      <div class="dw-badges">
        <div class="dw-badge">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
          <span>Async or Batch</span>
        </div>
        <div class="dw-badge">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="22 17 13.5 8.5 8.5 13.5 2 7"/><polyline points="16 17 22 17 22 11"/></svg>
          <span>Up to 10x cheaper</span>
        </div>
        <div class="dw-badge">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" x2="12" y1="15" y2="3"/></svg>
          <span>Stream results as ready</span>
        </div>
      </div>
      <div class="dw-cards">
        <a href="https://docs.doubleword.ai/batches/getting-started-with-batched-api" target="_blank" rel="noopener noreferrer" class="dw-card">
          <div class="dw-card-icon">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 7v14"/><path d="M3 18a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1h5a4 4 0 0 1 4 4 4 4 0 0 1 4-4h5a1 1 0 0 1 1 1v13a1 1 0 0 1-1 1h-6a3 3 0 0 0-3 3 3 3 0 0 0-3-3z"/></svg>
          </div>
          <div>
            <h3 class="dw-card-t">Getting Started</h3>
            <p class="dw-card-d">Learn how to run your first batch job</p>
          </div>
          <span class="dw-card-arrow">&rarr;</span>
        </a>
        <a href="https://docs.doubleword.ai/batches/autobatcher" target="_blank" rel="noopener noreferrer" class="dw-card">
          <div class="dw-card-icon">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 14a1 1 0 0 1-.78-1.63l9.9-10.2a.5.5 0 0 1 .86.46l-1.92 6.02A1 1 0 0 0 13 10h7a1 1 0 0 1 .78 1.63l-9.9 10.2a.5.5 0 0 1-.86-.46l1.92-6.02A1 1 0 0 0 11 14z"/></svg>
          </div>
          <div>
            <h3 class="dw-card-t">Coming from Real-Time?</h3>
            <p class="dw-card-d">Use Autobatcher to migrate existing API calls</p>
          </div>
          <span class="dw-card-arrow">&rarr;</span>
        </a>
        <a href="https://docs.doubleword.ai/batches/cli-examples" target="_blank" rel="noopener noreferrer" class="dw-card">
          <div class="dw-card-icon">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="18" x="3" y="3" rx="2" ry="2"/><circle cx="9" cy="9" r="2"/><path d="m21 15-3.086-3.086a2 2 0 0 0-2.828 0L6 21"/></svg>
          </div>
          <div>
            <h3 class="dw-card-t">Looking for inspiration?</h3>
            <p class="dw-card-d">Check out our use case examples</p>
          </div>
          <span class="dw-card-arrow">&rarr;</span>
        </a>
        <a href="https://calendly.com/meryem-pikk/30min" target="_blank" rel="noopener noreferrer" class="dw-card">
          <div class="dw-card-icon">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
          </div>
          <div>
            <h3 class="dw-card-t">Book a 15-minute Demo</h3>
            <p class="dw-card-d">Talk to an engineer</p>
          </div>
          <span class="dw-card-arrow">&rarr;</span>
        </a>
      </div>
    </div>
  </div>
`;
